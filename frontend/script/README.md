...
3. 从 broadcast JSON 文件（`contracts/broadcast/Deploy.s.sol/31337/run-latest.json`）解析合约地址
...
1. **jq 解析 broadcast JSON** — 从 `contracts` 字段提取（最可靠）
2. **grep 解析 broadcast JSON** — jq 不可用或解析失败时的兜底
3. **grep 解析 forge stdout** — broadcast 文件不可用时的最终兜底
